...
import default_injecting_validator
...
class ResourceProvider(object):
    ...
    def custom_cfn_resource_name(self):
        return f"Custom::{self.__class__.__name__.replace('Provider', '')}"
    ...
    def is_valid_cfn_request(self):
        ...
        try:
            ...
            self.fail(f'invalid CloudFormation Request received: {str(e.context)}')
            return False
    ...
    def is_valid_cfn_response(self):
        ...
        """
        try:
            jsonschema.validate(self.response, ResourceProvider.cfn_response_schema)
            return True
        except jsonschema.ValidationError as e:
            log.warning(f'invalid CloudFormation response created: {str(e)}')
            return False
    ...
    def is_valid_request(self):
        ...
        try:
            ...
            self.fail(f'invalid resource properties: {e.message}')
            return False
    ...
    def is_supported_request(self):
        ...
        if not supported:
            self.fail(f'ResourceType {str(self.resource_type)} not supported '
                      f'by provider {str(self.custom_cfn_resource_name)}')
        return supported
    ...
    def create(self):
        ...
        """
        self.fail(f'create not implemented by {self}')

    def update(self):
        """
        update the custom resource
        """
        self.fail(f'update not implemented by {self}')

    def delete(self):
        """
        delete the custom resource
        """
        self.success(f'delete not implemented by {self}')
    ...
    def handle(self, request, context):
        ...
        """
        try:
            log.debug(f'received request {json.dumps(request)}')
    ...
    def send_response(self):
        ...
        log.debug(f'sending response to {str(url)} ->  {json.dumps(self.response)}')
        r = requests.put(url, json=self.response, headers={'content-type': ''})
        if r.status_code != 200:
            raise Exception(f'failed to put the response to {url} status code {r.status_code}, {r.text}')
