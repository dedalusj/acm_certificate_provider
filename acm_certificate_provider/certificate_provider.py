...
class CertificateProvider(ResourceProvider):
    ...
    def update(self):
        ...
        for name in new_names.union(old_names).difference({'ServiceToken'}):
        ...
        try:
            if 'DomainName' in changed_properties:
                ...
                try:
                    ...
                    acm = boto3.client('acm', region_name=region)
                    acm.update_certificate_options(CertificateArn=self.physical_resource_id,
                                                   Options=self.get('Options'))
                except ClientError as error:
                    self.fail(f'{error}')
            elif changed_properties:
                self.fail(f'You can only change the "Options" and "DomainName" '
                          f'of a certificate, you tried to change {", ".join(changed_properties)}')
            else:
    ...
    def delete(self):
        ...
        try:
            region = self.properties.get('Region')
            acm = boto3.client('acm', region_name=region)
            acm.delete_certificate(CertificateArn=self.physical_resource_id)
        except ClientError as error:
            self.success(f'Ignore failure to delete certificate {error}')
